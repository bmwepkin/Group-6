...
plotResponseTime <- function(df, xcol, filename, flip = FALSE) {
  ...
  if (flip == TRUE) {
    df[[xcol]] <- factor(rev(levels(df[[xcol]])))
  }
...
plotResponseTimeByDivision <- function(df, filename, flip = FALSE) {
  ...
  p  <- ggplot(df, aes(x = Division, y = num_incidents, fill = response)) +
    ...
    scale_fill_brewer(palette = getMyPalette(), name = "Response Time", direction = -1) +
...
plotAgeGroupByRace <- function(df, name) {
  ...
  p  <- ggplot(df, aes(x = Age_Group, y = num_incidents, fill = Victim_Race)) +
    geom_col(position = position_dodge2()) +
    scale_fill_brewer(palette = getPaletteForRace(), name = "Race") +
...
plotViolentCrimesByAgeGroupAndGender <- function(df, name) {
  ...
  p  <- ggplot(df, aes(x = NIBRS_Crime_Category, y = num_incidents, fill = Victim_Gender)) +
    ...
    scale_fill_brewer(palette = getMyPalette(), name = "Gender", direction = 1) +
    my_theme()
...
plotByVictimRacesAsLollipop <- function(df, name, flip = FALSE) {
  ...
  p  <- ggplot(df, aes(x = Victim_Race, y = num_incidents)) +
    ...
    scale_color_brewer(palette = getPaletteForRace(), name = "Race") +
    scale_fill_brewer(palette = getMyPalette(), name = "Type", direction = -1) +
...
plotResponseTimeByDivisionOnMap <- function(df, filename) {
  # df <- ungroup(df) %>%
  #   group_by(Division, NIBRS_Crime_Category) %>%
  #   summarize(med_response = median(Response_Time)) %>%
  #   mutate(response = cut(med_response, breaks = quantile(med_response), include.lowest = TRUE, labels = c("Fastest", "Fast", "Slow", "Slowest")))

    # med_response = median(Response_Time)
    df <- mutate(df, response = cut(Response_Time, breaks = quantile(Response_Time), include.lowest = TRUE, labels = c("Fastest", "Fast", "Slow", "Slowest")))
    df <- filter(df, response == "Fastest")

    map_dallas_sat <- get_map('Dallas', zoom = 12, maptype = 'satellite')
    p <- ggmap(map_dallas_sat) + 
        geom_point(data = df, aes(x = long, y = lat, fill=response)) +
        # scale_fill_brewer(name = "Division", palette = getPaletteForRace()) +
        scale_fill_brewer(name="Type", palette =  getMyPalette(), direction = -1) +
        my_theme()
...
plotResidentsByDivisions <- function(df, filename) {
    p <- qplot(Division, data = df, geom = "bar", fill = Resident) +
        scale_fill_brewer(palette = getMyPalette()) +
        my_theme() +
        theme(axis.text.x = element_text(angle = -45)) +
        theme(axis.ticks.x = element_blank()) +
        theme(legend.title = element_blank())
...
main <- function() {
  ...
  violent_crime_latlon <- sub_vc_latlon(dataframe = violent_crimes)
  Crime_Categories <- sub_crime_categories()
  assault <- sub_assault(dataframe = violent_crimes)
  murder <- sub_homicide(dataframe = violent_crimes)
  robbery <- sub_robbery(dataframe = violent_crimes)
  
  # generate plots
  top_10_offenses(pname = "Top Offenses", file = "Top_Offenses", fwidth = 8)
  total_crime_plot(Police_Incidents, pname = "Total Crime", file = "Total_Crime")
  ...
  plotResponseTimeByDivision(Police_Incidents, "", "ByDivision")
  
  plotResidentsByDivisions(Police_Incidents, "ResidentsByDivision")
  plotCrimeHeatMap(Police_Incidents, "CrimeHeatMap")
  
  plotResponseTimeByDivisionOnMap(Police_Incidents, "ResponseTimeByMap")
}



plotResponseTimesOnMap <- function(df, filename) {
    map_dallas_sat <- get_map('Dallas', zoom = 12, maptype = 'satellite')
    p <- ggmap(map_dallas_sat) + stat_density2d(data = df, aes(x = long, y = lat, fill = ..density..), geom = 'tile', contour = F, alpha = .5) +
        scale_fill_viridis_c(option = "inferno") +
        labs(fill = str_c('No. of', '\nIncidents')) +
...
plotViolentCrimeHeatMap1 <- function(df, filename) {
    map_dallas_sat <- get_map('Dallas', zoom = 12, maptype = 'satellite')
    p <- ggmap(map_dallas_sat) + stat_density2d(data = df, aes(x = long, y = lat, fill = ..density..), geom = 'tile', contour = F, alpha = .5) +
        scale_fill_viridis_c(option = "inferno") +
        labs(fill = str_c('No. of', '\nIncidents')) +
...
todo <- function() {
  ...
  m1 <- qplot(murder$`Time_of_Occurrence`, xlab="Timeof day", main= "Murders by time of day")
